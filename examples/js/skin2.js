...
window.addEventListener("load", function() {
  ...
  scene.mainCamera.location.set(0, 0.7, 1.2);
  ...
  const skinObj1 = loadSkinData('IMG_6365_720p', 274, 'yellow');
  const skinObj2 = loadSkinData('IMG_1924_720p', 0, 'blue');
  let transformSkin = new SkinRenderObject('white');
      ...
      function convertJoint(jointArray) {
        for (let i = 0; i < frameCount; i++) {
          ...
          for (let j = 0; j < skinBodyFrame1.length; j++) {
            ...
            const v1 = skinBody1.v, v2 = skinBody2.v;
...
function loadSkinData(filename, frameOffset = 0, skinColor = 'white') {
  ...
  const unprojection = new Matrix4().perspective(60, 720 / 1280, 0.1, 100).inverse();
