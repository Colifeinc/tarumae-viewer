...
window.addEventListener("load", function() {
  ...
  let timer, isStop = true, playSpeed = 50;
  ...
  const constSkels = [
    ...
    // { name: 'upper_arm.R.001', type: 'const', q: Quaternion.fromEuler(new Vec3(-77,27,20)) },
    { name: 'upper_arm.R.002', type: 'const', q: Quaternion.fromEuler(new Vec3(150,31,-13))  },
    { name: 'upper_arm.R.003', type: 'const', q: Quaternion.fromEuler(new Vec3(135,23,2))  },

    // { name: 'forearm.R.002', type: 'angle' },
    // { name: 'forearm.R.002', type: 'const', q: Quaternion.fromEuler(new Vec3(-77,27,20)) },
  ...
  let seizaWeight = 0, r1 = 0, headAngle = new Vec3();
  ...
  function updateJoints() {
    ...
    if (poseData) {
      const translateSkeleton = (skel) => {
        ...
        if (baseJoint.score < minScore) return;
    ...
    for (const cskel of constSkels) {
      ...
      //   const wq = Quaternion.fromRotationMatrix(cskel.obj.parent.jointWorldRotation).inverse();
      //   let q = Quaternion.mul(wq, cskel.q);
      //   cskel.obj._quaternion.set(q);
      // }
